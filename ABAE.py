...
parser.add_argument('--img_size', type=int, default=32, help='img size of smallimagenet127')
args = parser.parse_args()

state = {k: v for k, v in args._get_kwargs()}
if args.dataset=='cifar10':
    import dataset.fix_cifar10 as dataset
    import wrn_graph as models
...
def main():
    ...
    labeled_trainloader = data.DataLoader(train_labeled_set, batch_size=args.batch_size, shuffle=True, num_workers=8,
                                          drop_last=True)
    unlabeled_trainloader = data.DataLoader(train_unlabeled_set, batch_size=args.batch_size, shuffle=True, num_workers=8,drop_last=True)
    test_loader = data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False, num_workers=8)

    # Model
    print("==> creating WRN-28-2 with abae")
    ...
    def update1(model, flag = True):
        paras = []
        for name, p in model.named_parameters():
            if "linear_d" not in name:
                print("para1_update_only", name)
                p.reqires_grad = flag
                paras.append(p)
        return paras

    def update2(model, flag = True):
        paras = []
        for name, p in model.named_parameters():
            if "linear_d" in name:
                print("para2_update_only", name)
                p.reqires_grad = flag
                paras.append(p)
        return paras
    ...
    paras1 = update1(model, flag = True)
    paras2 = update2(model, flag = True)
    optimizer = optim.Adam(paras1, lr=args.lr)
    optimizer2 = optim.Adam(paras2, lr=args.lr)
    ema_optimizer = WeightEMA(model, ema_model, alpha=args.ema_decay)
    start_epoch = 0

    # Resume
    title = 'ABAEfix-' + args.dataset
    if args.resume:
        # Load checkpoint.
        ...
        logger.set_names(['Train Loss', 'Train Loss X', 'Train Loss U', 'abaeloss','Test Loss', 'Test Acc','Test last half ACC.'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Training part
        train_loss, train_loss_x, train_loss_u, abaeloss = train(labeled_trainloader,
                                                                                                unlabeled_trainloader,
                                                                                                model, optimizer, optimizer2, 
                                                                                                ema_optimizer,
                                                                                                train_criterion,
                                                                                                epoch,ir2)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, optimizer2, ema_optimizer, criterion, epoch,ir2):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    losses_x = AverageMeter()
    losses_u = AverageMeter()
    losses_abae = AverageMeter()
    end = time.time()
    ...
    for batch_idx in range(args.val_iteration):
        ...
        select_mask = max_p.ge(0.95).float()
        ...
        logit_x,logit_si,logit = model.classify2(q)
        _,_,logitu1 = model.classify2(q1)
        _,_,logitu2 = model.classify2(q2)
        _,_,logitu3 = model.classify2(q3)

        mse0 = torch.mean(torch.square(logit_x-logit_si), dim=1)
    
        logits = F.softmax(logit)

        logitsu1 = F.softmax(logitu1)
        max_p2, label_u = torch.max(logitsu1, dim=1)
        select_mask2 = max_p2.ge(0.95) + 1.0 - (epoch/args.epochs)
        ...
        thrd_mse = 0.001*torch.sum(targets_x2 * torch.tensor(ir22).cuda(0), dim=1).detach()
        zeros_tag = torch.zeros_like(mse0, dtype=torch.float64)
        loss_ae = torch.mean(torch.where((mse0-thrd_mse)>0.0,(mse0-thrd_mse),zeros_tag))

        logitsu2 = F.softmax(logitu2)
        logitsu3 = F.softmax(logitu3)
       
        abaeloss_rce = -torch.mean(maskforbalance * torch.sum(torch.log(torch.clamp(logits,1e-8,1.0)) * targets_x2.cuda(0), dim=1))

        abaeloss_cce1 = -torch.mean(
            select_mask2 * maskforbalanceu * torch.sum(torch.log(torch.clamp(logitsu2,1e-8,1.0)) * logitsu1.cuda(0).detach(), dim=1))

    
        abaeloss_cce2 = -torch.mean(
            select_mask2 * maskforbalanceu * torch.sum(torch.log(torch.clamp(logitsu3,1e-8,1.0)) * logitsu1.cuda(0).detach(), dim=1))
       
        abaeloss=abaeloss_rce+(abaeloss_cce1+abaeloss_cce2)
        Lx, Lu = criterion(logits_x, all_targets[:batch_size], logits_u, all_targets[batch_size:], select_mask)
        loss = Lx + Lu+ abaeloss  
        losses.update(loss.item(), inputs_x.size(0))
        losses_x.update(Lx.item(), inputs_x.size(0))
        losses_u.update(Lu.item(), inputs_x.size(0))
        losses_abae.update(abaeloss.item(), inputs_x.size(0))
        ...
        bar.suffix  = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | ' \
                    ...
                    loss_x=losses_x.avg,
                    loss_u=losses_u.avg,
            loss_m=losses_abae.avg,
                    )
        bar.next()
    bar.finish()

    return (losses.avg, losses_x.avg, losses_u.avg, losses_abae.avg)
...
def validate(valloader, model,criterion, mode):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(valloader):
            ...
            q=model(inputs)
            _,_,outputs2=model.classify2(q)
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, outputs_u, targets_u, mask):
        ...
        #Lu2 = -torch.mean(torch.sum(F.log_softmax((1-outputs_u), dim=1) * (1-targets_u), dim=1) * (1-mask))
        return Lx, Lu#+Lu2
