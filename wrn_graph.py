...
class GPRGNN(torch.nn.Module):
    ...
    def forward(self, x):
        x_sis = self.linear_d(x)
        x_c = self.linear_c(x_sis)
        A_hat = torch.softmax(x @ x.T, dim=-1) + torch.eye(x.shape[0], device=x.device) 
        out = self.prop(x_c, A_hat)
...
class WideResNet(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=2, dropRate=0.0, K=3):
        ...
        self.graph_model = GPRGNN(K=1, input_dim=nChannels[3], d_model=256, output_dim=num_classes) #256 for cifar100
